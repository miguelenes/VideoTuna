...
def get_loraconfig(transformer, rank=128, alpha=128, init_lora_weights="gaussian"):
    ...
    for name, module in transformer.named_modules():
        if (
            "blocks" in name
            and "face" not in name
            and "modulation" not in name
...
class TensorList(object):
    ...
    def _apply(self, other, op):
        if isinstance(other, (list, tuple, TensorList)) or (
            isinstance(other, torch.Tensor) and (other.numel() > 1 or other.ndim > 1)
        ):
            ...
            isinstance(other, torch.Tensor) and (other.numel() == 1 and other.ndim <= 1)
        ):
