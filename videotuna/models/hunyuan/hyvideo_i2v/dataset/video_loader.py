...
class VideoDataset(Dataset):
    def __init__(
        ...
        for json_file in json_files:
            with open(
                f"{data_jsons_path}/{json_file}", "r", encoding="utf-8-sig"
            ) as file:
                data = json.load(file)
        ...
        s_time = time.time()
        logger.info(
            f"load {data_jsons_path} \t cost {time.time() - s_time} s \t total length {len(self.table)}"
        )
    ...
    def get_batch(self, idx):
        ...
        if self.text_encoder_2 is None:
            return (
                ...
                text_mask.clone(),
                {
                    k: torch.as_tensor(v) if not isinstance(v, str) else v
...
if __name__ == "__main__":
    ...
    dataset = VideoDataset(args=None, data_jsons_path=data_jsons_path)
