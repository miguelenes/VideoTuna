...
def load_inputs_i2v(input_dir, video_size=(256, 256), video_frames=16):
    ...
    if len(prompt_files) > 1:
        ...
        print(
            f"Warning: multiple prompt files exist. The one
                {os.path.split(prompt_files[0])[1]} is used."
        )
...
def load_video_batch(
    ...
    2. when the total video frames is less than required, padding strategy will be used
        (repreated last frame)
    """
    fps_list = []
    batch_tensor = []
    ...
    for filepath in filepath_list:
        ...
        if max_valid_frames < required_frames:
            ...
            print(
                f"{os.path.split(filepath)[1]} is not long enough: {padding_num} frames
                    padded."
            )
