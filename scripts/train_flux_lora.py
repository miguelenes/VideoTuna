...
def main(args: argparse.Namespace) -> None:
    try:
        ...
        logger.warning(
            "Could not set multiprocessing start method to 'fork': %s", exc
        )
    run_training(args.config_path, args.data_config_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune Flux LoRA (Diffusers + PEFT)")
    parser.add_argument("--config_path", type=str, required=True, help="Training config JSON")
    parser.add_argument(
        "--data_config_path",
        type=str,
        required=True,
